...
사용자의 지시나 작업 결과로 규칙·컨벤션이 바뀌면 적절한 문서를 찾아 업데이트하거나 반영할 때 업데이트한다.

- **`DESIGN.md` 업데이트:** 디자인 시스템, 공통 스타일(색상/폰트), 메타데이터 스키마에 대한 변경이나 새로운 시각화 규칙이 추가될 때
- **`CLAUDE.md` 업데이트:** 코드 작성 규칙, 아키텍처 원칙, 개발 방법론, 폴더구조, 에러 처리 방식 및 코딩 스탠다드의 변경이 있을 때

---

## JS 라이브러리 관리

- **설치 전 확인**: `package.json`의 기존 의존성을 먼저 확인한다. 이미 설치된 라이브러리가 있으면 새로 설치하지 않는다.
- **공유 라이브러리**: 여러 프레젠테이션에서 공통으로 사용하는 JS 라이브러리는 npm 패키지로 설치한다 (`package.json` 업데이트).
- **설치 기록**: 새 라이브러리를 설치하면 `DESIGN.md`의 **JS 라이브러리 인벤토리** 테이블을 업데이트한다.
- **미디어 대체**: 코드로 구현하기 어려운 복잡한 이미지·영상이 필요한 경우, 직접 구현하지 말고 `src/presentations/<id>/media/` 폴더에 애셋을 넣도록 사용자에게 안내한다.
